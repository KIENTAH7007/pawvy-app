...
export const reportsApi   = { pnl: (q) => api.get(`/reports/pnl${qs(q)}`), trend: (q) => api.get(`/reports/trend${qs(q)}`), partners: (q) => api.get(`/reports/partners${qs(q)}`), allChannels: (q) => api.get(`/reports/all-channels${qs(q)}`), upsell: () => api.get('/reports/upsell'), channelPerformance: (q) => api.get(`/reports/channel-performance${qs(q)}`) };
